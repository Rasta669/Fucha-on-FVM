...
var FunctionFragment = class extends NamedFragment {
  ...
  static from(obj) {
    ...
    return new FunctionFragment(_guard4, obj.name, obj.stateMutability, obj.inputs ? obj.inputs.map(ParamType.from) : [], obj.outputs ? obj.outputs.map(ParamType.from) : [], obj.gas != null ? obj.gas : null);
  }
  static isFragment(value) {
...
var WrappedFallback = class {
  ...
  async populateTransaction(overrides) {
    ...
    assertArgument(payable || (tx.value || BN_010) === BN_010, "cannot send value to non-payable contract", "overrides.value", tx.value);
...
var WrappedMethod = class extends _WrappedMethodBase() {
...
var WrappedEvent = class extends _WrappedEventBase() {
  constructor(contract, key) {
    super();
    __publicField(this, "name", "");
    // @TODO: investigate 
...
var _subs, _plugins2, _pausedState, _networkPromise, _anyNetwork, _performCache, _lastBlockNumber, _nextTimer, _timers, _disableCcipRead, _perform, perform_fn, _call, call_fn, _checkNetwork, checkNetwork_fn, _getAccountValue, getAccountValue_fn, _getBlock, getBlock_fn, _hasSub, hasSub_fn, _getSub, getSub_fn;
...
var _options2, _nextId, _payloads, _drainTimer, _notReady, _network2, _scheduleDrain, scheduleDrain_fn;
...
var SocketEventSubscriber = class extends SocketSubscriber {
  ...
  async _emit(provider, message) {
    provider.emit(__privateGet(this, _logFilter), provider._wrapLog(message, provider._network));
  }
};
  ...
  switch (req.method) {
    case "getBlockNumber": {
      if (__privateGet(this, _height) === -2) {
        __privateSet(this, _height, Math.ceil(getNumber(getMedian(this.quorum, __privateGet(this, _configs).map((c) => ({
          value: c.blockNumber,
